...
document.getElementById('opportunityForm').addEventListener('submit', function(e) {
    ...
    // REAL BACKEND CALL
    fetch('/opp/api/opportunities', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include', // Needed to prove the admin is logged in
        body: JSON.stringify({
            name: name,
            duration: duration,
    ...
    .then(res => {
        if (res.status === 201) {
            ...
            showToast('Error: ' + (res.body.error || 'Could not save opportunity'));
        }
    })
    .catch(err => showToast('Cannot connect to database.'));
});
   ...
   fetch('/auth/api/login', {
    ...
    .then(res => {
        if (res.status === 200) {
            ...
            showError('loginPasswordErr', res.body.error || 'Invalid credentials');
            shakeForm('loginForm');
        }
        generateCaptcha('login');
    })
...
if (!valid) { shakeForm('signupForm'); return; }
    ...
    .then(res => {
        if (res.status === 201) {
            ...
            showError('signupEmailErr', res.body.error || 'Signup failed');
